-- ═══════════════════════════════════════════════════════════════
-- FIELDFRESH — CLEAN DATABASE SETUP
...
DROP FUNCTION IF EXISTS get_nearby_customer_tokens CASCADE;
DROP FUNCTION IF EXISTS expire_old_products CASCADE;
DROP FUNCTION IF EXISTS update_farmer_wallet CASCADE;
...
CREATE INDEX idx_products_valid_until ON products(valid_until);
...
-- Users policies - Allow public read and insert for registration
...
-- Orders policies
CREATE POLICY "Customers can read own orders" ON orders FOR SELECT USING (auth.uid() = customer_id OR auth.uid() = farmer_id);
CREATE POLICY "Customers can create orders" ON orders FOR INSERT WITH CHECK (auth.uid() = customer_id);
CREATE POLICY "Farmers can update order status" ON orders FOR UPDATE USING (auth.uid() = farmer_id OR auth.uid() = customer_id);
...
-- Wallets policies
CREATE POLICY "Farmers can read own wallet" ON farmer_wallets FOR SELECT USING (auth.uid() = farmer_id);
...
INSERT INTO users (id, phone, name, role, latitude, longitude, is_verified, is_kyc_done, rating, created_at) VALUES
  ...
  ('aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '+919999999999', 'Admin', 'admin', NULL, NULL, true, true, 5.0, NOW());
